...
const packCreateChannel = [
    config.packChannelId
];
    ...
    if (interaction instanceof CommandInteraction) {
        ...
        switch(subcommand.name){
            ...
            case 'unlink':
                ...
                break;
            case "invite":
                const inviteAmount = subcommand.options?.find((option)=>option.name === 'a'
                )?.value;
                const madfutuser = subcommand.options?.find((option)=>option.name === 'u'
                )?.value;
                const inviteCoins = subcommand.options?.find((option)=>option.name === 'c'
                )?.value ?? 0;
                const invitePacks = subcommand.options?.find((option)=>option.name === 'p'
                )?.value?.split(".").filter((el)=>el.length
                );
            ...
            case 'create':
                {
                    ...
                    const username1 = subcommand.options?.find((option)=>option.name === 'username'
                    )?.value ?? "";
                    const amount = subcommand.options?.find((option)=>option.name === 'amount'
                    )?.value ?? undefined;
                    const packType = subcommand.options?.find((option)=>option.name === 'type'
                    )?.value ?? "";
                    const customName = subcommand.options?.find((option)=>option.name === 'name'
                    )?.value ?? "";
                    const minPackRating = subcommand.options?.find((option)=>option.name === 'min'
                    )?.value ?? undefined;
                    const maxPackRating = subcommand.options?.find((option)=>option.name === 'max'
                    )?.value ?? undefined;
                    const teamId = subcommand.options?.find((option)=>option.name === 'team'
                    )?.value ?? undefined;
                    const nation = subcommand.options?.find((option)=>option.name === 'nation'
                    )?.value ?? undefined;
                    const percentage = subcommand.options?.find((option)=>option.name === 'percentage'
                    )?.value ?? undefined;
                    const leauge = subcommand.options?.find((option)=>option.name === 'leauge'
                    )?.value ?? undefined;
            ...
            case 'flip':
                ...
                break;
            case 'linking':
                exportedBot.emit("linking", interaction, subcommand.options.find((option)=>option.name === 'username'
