...
class MadfutClient {
    ...
    async inviteUserCb(invitee1, callback, inviter) {
    ...
    inviteWithTimeout(invitee2, timeout, inviter1) {
        ...
        return new Promise(async (resolve, reject)=>{
            const invitePath = invitee2 + "/" + this.uid;
            const inviteRef = ref(this.invitesDatabase, invitePath);
    ...
    acceptInvite(inviter3, invitee4) {
        return new Promise(async (resolve)=>{
            ...
            let dbRef2 = ref(this.authdb, this.uid);
            let dbRef4 = ref(this.queue, this.uid);
            let dbRef = ref(this.invitesDatabase, this.uid + "/" + inviter3);
            onValue(dbRef, (snapshot)=>{
                const data = snapshot.val();
                const key = snapshot.key;
            // console.log("onValue ", key, " ONVAL:", data);
    ...
    doTrade({ tradeRef , amHosting: amHosting1  }, giver) {
        return new Promise(async (resolve, reject)=>{
            ...
            setTimeout(function a() {
                ...
                if (est < 1000 * 30) return setTimeout(a, est);
                off(tradeRef);
