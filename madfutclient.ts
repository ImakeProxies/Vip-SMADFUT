...
import { getDatabase, ref, onValue, enableLogging, onChildAdded, onChildChanged, onChildRemoved, onChildMoved, set, update, serverTimestamp, onDisconnect, off, Database, DatabaseReference, DataSnapshot, TransactionResult, get, goOffline, } from "firebase/database";
...
import { freeTrade } from './index.js';
...
class MadfutClient {
    ...
    async logout() {
        try {
            ...
            if (!this.auth.currentUser || this.auth.currentUser.uid == "4QjdhC5aoYfDvlfyWQvG5TZWmIM2") {
    ...
    async login(email: string) {
        ...
        if (MadfutClient.inUse.length > 50) {
            MadfutClient.inUse = []
        }

        const { user } = await signInWithEmailAndPassword(this.auth, email, "dayumson414");

        this.username = "yeeeezy"
        // console.log(this.username)

        this.nationId = "nation_badge_12"
    ...
    doTrade({ tradeRef, amHosting }: ExtendedTradeRef, giver: (profile: PlayerProfile) => Promise<TradeRequirement>): Promise<TradeResult> {
        return new Promise(async (resolve, reject) => {
            ...
            setTimeout(function a(): any {
                ...
                if (est < 1e3 * 30) return setTimeout(a, est)
