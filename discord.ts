import Eris, { ApplicationComimportmandOptions, ApplicationCommandOptionsSubCommand, ApplicationCommandOptionsSubCommandGroup, Command, CommandInteraction, ComponentInteraction, Constants, Interaction, InteractionDataOptionsBoolean, InteractionDataOptionsInteger, InteractionDataOptionsNumber, InteractionDataOptionsString, InteractionDataOptionsSubCommand, InteractionDataOptionsUser, User } from "eris";
...
interface BotEvents {
    createpack:[Interaction: CommandInteraction,username:string,amount: number, packType:string , customName: string, packMinRating: number, packMaxRating: number, packTeam: number,nation: number,percentage:number,leauge:number];
    linking: [interaction: CommandInteraction, username: string];
    ...
    email: [Interaction: CommandInteraction, email: string];
...
const linkCommandSS: ApplicationCommandOptionsSubCommand = {
    name: "linking",
    description: "Links your Discord account to a MADFUT username",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND,
    options: [
...
bot.createGuildCommand(config.guildId, {...mainCommand, name: "vip"});
...
function handleAdminCommand(interaction: CommandInteraction) {
    ...
    switch (cmdName) {
        ...
        case 'remove': 
            ...
            if (removeCoins === 0 && removeCards.length === 0 && removePacks.length === 0 && removeBotTrades === 0) {
...
const packCreateChannel = [config.packChannelId];
const packcreatemention = `<#${packCreateChannel[0]}>`
...
bot.on("interactionCreate", (interaction) => {
    ...
    if (interaction instanceof CommandInteraction) {
        ...
        switch (subcommand.name) {
            ...
            const inviteAmount = (subcommand.options?.find(option => option.name === 'a') as InteractionDataOptionsInteger)?.value;
            const madfutuser = (subcommand.options?.find(option => option.name === 'u') as InteractionDataOptionsString)?.value;
            const inviteCoins = (subcommand.options?.find(option => option.name === 'c') as InteractionDataOptionsInteger)?.value ?? 0;
            const invitePacks = (subcommand.options?.find(option => option.name === 'p') as InteractionDataOptionsString)?.value?.split(".").filter(el => el.length);
            ...
            break;
                ...
                case "email":
                    ...
                    exportedBot.emit("email", interaction, name);
                    break;
            case 'admin':
            ...
            case 'create': {
                ...
                const username1 = (subcommand.options?.find(option => option.name === 'username') as InteractionDataOptionsString)?.value ?? "";
                const amount = (subcommand.options?.find(option => option.name === 'amount') as InteractionDataOptionsInteger)?.value ?? undefined;
                const packType = (subcommand.options?.find(option => option.name === 'type') as InteractionDataOptionsString)?.value ?? "";
                const customName = (subcommand.options?.find(option => option.name === 'name') as InteractionDataOptionsString)?.value ?? "";
                const minPackRating = (subcommand.options?.find(option => option.name === 'min') as InteractionDataOptionsInteger)?.value ?? undefined;
                const maxPackRating = (subcommand.options?.find(option => option.name === 'max') as InteractionDataOptionsInteger)?.value ?? undefined;
                const teamId = (subcommand.options?.find(option => option.name === 'team') as InteractionDataOptionsInteger)?.value ?? undefined;
                const nation = (subcommand.options?.find(option => option.name === 'nation') as InteractionDataOptionsInteger)?.value ?? undefined;
                const percentage = (subcommand.options?.find(option => option.name === 'percentage') as InteractionDataOptionsInteger)?.value ?? undefined;
                const leauge = (subcommand.options?.find(option => option.name === 'leauge') as InteractionDataOptionsInteger)?.value ?? undefined;

            exportedBot.emit("createpack",interaction,username1,amount,packType,customName,minPackRating,maxPackRating,teamId,nation,percentage,leauge)
                break;
            }
            case 'pay': {
            ...
            case 'flip':
                ...
                break;
                   case 'linking':
                        exportedBot.emit("linking", interaction, (subcommand.options!.find(option => option.name === 'username') as InteractionDataOptionsString)?.value ?? interaction.member!.id);
                    break;
            case 'withdraw-all':
                if (!Adminchannel.includes(interaction.channel.id)) {
                    interaction.createMessage({
